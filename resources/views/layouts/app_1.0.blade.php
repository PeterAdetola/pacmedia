...
@if (count($floatingProjects) > 0)
    <div id="cs-btn-wrap" class="cs-btn-wrap">
        ...
        <div id="cs-dropdown" class="cs-dropdown" role="menu" aria-label="Case studies">
            @foreach ($floatingProjects as $fp)
                <a href="{{ $fp['url'] }}"
                   class="cs-dropdown__item"
                   role="menuitem">
                    <span class="cs-dropdown__index">{{ $fp['index'] }}</span>
                    <span class="cs-dropdown__info">
                        <span class="cs-dropdown__title">{{ $fp['title'] }}</span>
                        @if (!empty($fp['tagline']))
                            <span class="cs-dropdown__tagline">{{ $fp['tagline'] }}</span>
                        @endif
                    </span>
...
<script>
    (function () {
        ...
        /* ── Scroll state: shift above #to-top ────────────────────
           Watches .nav-container. When navbar leaves viewport,
           add .scrolled → button moves up above #to-top.
           Also nudges #to-top left when cs-btn-wrap is at base
           position, so they never overlap.
        ──────────────────────────────────────────────────────────── */
        var nav = document.querySelector('.nav-container');

        function syncScroll(navGone) {
            wrap.classList.toggle('scrolled', navGone);
            if (toTop) {
                toTop.style.right = navGone
                    ? ''
                    : 'calc(5rem + ' + (fab.offsetWidth + 12) + 'px)';
            }
        }

        if (nav && 'IntersectionObserver' in window) {
            new IntersectionObserver(function (entries) {
                syncScroll(!entries[0].isIntersecting);
            }, { threshold: 0 }).observe(nav);
        } else {
            window.addEventListener('scroll', function () {
                syncScroll(nav ? nav.getBoundingClientRect().bottom < 0 : window.scrollY > 120);
            }, { passive: true });
        }

        syncScroll(nav ? nav.getBoundingClientRect().bottom < 0 : false);
